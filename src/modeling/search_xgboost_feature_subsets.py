"""
Description:
    상관성이 높은 변수 제거 조합별로 XGBoost를 튜닝하고 성능과 중요도를 비교합니다.

Author:
    박준수

Source:
    Notion ML database, page 37cbf34c-c2ef-8090-8cb1-d2482dd79846
"""
...
print("\n[ 🚀 XGBoost를 활용한 블루리본 예측 최종 모델링 (과적합 방지 Optuna 튜닝) ]")
...
# 이미지 모델 결과 컬럼들의 0 값을 NaN으로 변경
# XGBoost도 결측치를 자체적으로 처리할 수 있으므로 NaN 유지
image_cols = [
...
print("\n[ 🔁 상관계수 높은 변수쌍 기준 32개 조합 실험 시작 - XGBoost 버전 ]")
...
for combo_idx, combo in enumerate(all_combinations, start=1):
    ...
    print("\n" + "=" * 90)
    print(f"[진행상황] {combo_idx} / {len(all_combinations)} 번째 조합 실행 중 - XGBoost")
    print("=" * 90)
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name=f"XGB_combo_{combo_idx}"
    )

    study.optimize(
        objective_xgb_regularized,
        n_trials=N_TRIALS,
    ...
    best_xgb_model.fit(X_train_sub, y_train)

    # ---------------------------------------------------------------------
    # 4-6. Train / Test 예측
    # ---------------------------------------------------------------------

    y_train_pred = best_xgb_model.predict(X_train_sub)
    y_test_pred = best_xgb_model.predict(X_test_sub)

    y_train_proba = best_xgb_model.predict_proba(X_train_sub)[:, 1]
    y_test_proba = best_xgb_model.predict_proba(X_test_sub)[:, 1]
...
print("\n" + "=" * 90)
print("🏆 XGBoost 32개 조합 전체 결과 Top 10")
print("=" * 90)
...
results_df_sorted.to_csv(
    "../data/xgb_상관변수_32조합_제거실험결과.csv",
    index=False,
    encoding="utf-8-sig"
)

print("\n✅ 결과 저장 완료: ../data/xgb_상관변수_32조합_제거실험결과.csv")

# =========================================================================
# 6. 최종 1등 조합 변수 중요도 확인 - XGBoost
# =========================================================================

best_combo_num = int(results_df_sorted.loc[0, "조합번호"])

best_info = best_models[best_combo_num]
...
print("\n" + "=" * 90)
print(f"🏆 최종 선택 조합 번호: {best_combo_num}")
print("=" * 90)

print("\n제거된 변수:")
for col in best_info["drop_cols"]:
    print(f" - {col}")
...
plt.title(f"XGBoost 최종 선택 조합 {best_combo_num} 변수 중요도 Top 20")
plt.xlabel("중요도")
plt.ylabel("변수명")
plt.tight_layout()
plt.show()
...
plt.title("XGBoost 32개 변수 제거 조합별 Test ROC-AUC 비교")
plt.xlabel("조합번호")
...
plt.title("XGBoost 32개 변수 제거 조합별 Train-Test AUC Gap 비교")
plt.xlabel("조합번호")
plt.ylabel("AUC Gap")
plt.xticks(rotation=45)
plt.tight_layout()
...
print(f"최종 선택 조합 번호: {best_combo_num}")
print(f"최종 Test ROC-AUC : {results_df_sorted.loc[0, 'Test_ROC_AUC']:.4f}")
print(f"최종 Train ROC-AUC: {results_df_sorted.loc[0, 'Train_ROC_AUC']:.4f}")
print(f"최종 AUC Gap      : {results_df_sorted.loc[0, 'AUC_Gap']:.4f}")
print(f"최종 Test F1      : {results_df_sorted.loc[0, 'Test_F1']:.4f}")
